...
(function (root, factory) {
    ...
    var browser = {
        ...
        // Flag for IE < 8.
        isIE: function () {
            if (browser.supports('filter') && !browser.supports('opacity')) browser.isIE = function () {
                return true;
            };
            else browser.isIE = function () {
                return false;
            };
            return browser.isIE();
        },
                ...
                if (browser.styleEl[domprop] !== undefined) {
                    ...
                    for (i = 0; i < browser.prefixes.length; i++) {
                        if (browser.styleEl[browser.prefixes[i] + domprop] !== undefined) {
                            browser.cache[prop] = {
                                css: '-' + browser.prefixes[i].toLowerCase() + '-' + prop,
                                dom: browser.prefixes[i] + domprop
                            };
                        }
                    }
                }
        ...
        add: {
            ...
            'linear': function (name, type, tStart, tEnd, oStart, oEnd) {
                browser.createKeyframe(name, {
                    '0': {
                        'transform': 'translate' + tStart[0] + '(0%)',
            ...
            'cube': function (name, rStart, rEnd, tZ, oStart, oEnd) {
                browser.createKeyframe(name, {
                    '0': {
                        'transform': 'rotate' + rStart + '0deg) translateZ(' + tZ + 'px)', 'opacity': oStart,
                        'visibility': 'visible'
                    },
                    '100': {
                        'transform': 'rotate' + rEnd + '0deg) translateZ(' + tZ + 'px)', 'opacity': oEnd,
                        'visibility': 'hidden'
                    }
                });
            }
        ...
        classnames: function (cls) {
            return {
                ...
                data: 'data-slidr-' + cls,
                id: function (_, css) {
                    return css ? 'aside[id="' + _.id + '-' + cls + '"]' : _.id + '-' + cls;
                }
            }
        },
    ...
    var controls = {
        ...
        onclick: function (_) {
            return function handler(e) {
                actions.slide(_, getattr(browser.sanitize(e).target, controls.cls.data));
            };
        },
    ...
    var breadcrumbs = {
        ...
        onclick: function (_) {
            return function handler(e) {
                actions.slide(_, getattr(browser.sanitize(e).target, breadcrumbs.cls.data));
            };
        },
    ...
    var fx = {
        ...
        animation: {
            ...
            'linear': {
                'in': {
                    'left': function (name, w, o) {
                        browser.add['linear'](name, 'in', 'X(-' + w, 'X(0', o, '1');
                    },
                    'right': function (name, w, o) {
                        browser.add['linear'](name, 'in', 'X(' + w, 'X(0', o, '1');
                    },
                    'up': function (name, h, o) {
                        browser.add['linear'](name, 'in', 'Y(-' + h, 'Y(0', o, '1');
                    },
                    'down': function (name, h, o) {
                        browser.add['linear'](name, 'in', 'Y(' + h, 'Y(0', o, '1');
                    }
                },
                'out': {
                    'left': function (name, w, o) {
                        browser.add['linear'](name, 'out', 'X(0', 'X(' + w, '1', o);
                    },
                    'right': function (name, w, o) {
                        browser.add['linear'](name, 'out', 'X(0', 'X(-' + w, '1', o);
                    },
                    'up': function (name, h, o) {
                        browser.add['linear'](name, 'out', 'Y(0', 'Y(' + h, '1', o);
                    },
                    'down': function (name, h, o) {
                        browser.add['linear'](name, 'out', 'Y(0', 'Y(-' + h, '1', o);
                    }
            ...
            'cube': {
                'in': {
                    'left': function (name, w, o) {
                        browser.add['cube'](name, 'Y(-9', 'Y(', w / 2, o, '1');
                    },
                    'right': function (name, w, o) {
                        browser.add['cube'](name, 'Y(9', 'Y(', w / 2, o, '1');
                    },
                    'up': function (name, h, o) {
                        browser.add['cube'](name, 'X(9', 'X(', h / 2, o, '1');
                    },
                    'down': function (name, h, o) {
                        browser.add['cube'](name, 'X(-9', 'X(', h / 2, o, '1');
                    }
                ...
                'out': {
                    'left': function (name, w, o) {
                        browser.add['cube'](name, 'Y(', 'Y(9', w / 2, '1', o);
                    },
                    'right': function (name, w, o) {
                        browser.add['cube'](name, 'Y(', 'Y(-9', w / 2, '1', o);
                    },
                    'up': function (name, h, o) {
                        browser.add['cube'](name, 'X(', 'X(-9', h / 2, '1', o);
                    },
                    'down': function (name, h, o) {
                        browser.add['cube'](name, 'X(', 'X(9', h / 2, '1', o);
                    }
        ...
        animate: function (_, el, trans, type, dir, opt_target, opt_z, opt_pointer) {
            ...
            if (browser.isIE()) anim = extend(anim, {
                'display': (type === 'in') ? 'block' : 'none',
                'visibility': 'visible'
            });
    ...
    var size = {
        ...
        dynamic: function (_) {
            var clone = css(_.slidr.cloneNode(false), {
                'position': 'absolute',
                'opacity': '0',
                'filter': 'alpha(opacity=0)'
            });
        ...
        widthPad: function (el) {
            return size.sum(css(el, 'padding-left'), css(el, 'padding-right'));
        },
        heightPad: function (el) {
            return size.sum(css(el, 'padding-top'), css(el, 'padding-bottom'));
        },

        // Grabs the element width/height border.
        widthBorder: function (el) {
            return size.sum(css(el, 'border-left-width'), css(el, 'border-right-width'));
        },
        heightBorder: function (el) {
            return size.sum(css(el, 'border-top-width'), css(el, 'border-bottom-width'));
        },

        // Grabs extra border-box padding.
        extraWidth: function (el) {
            return size.sum(size.widthPad(el), size.widthBorder(el));
        },
        extraHeight: function (el) {
            return size.sum(size.heightPad(el), size.heightBorder(el));
        },
        ...
        mouse: {
            ...
            track: function (el) {
                bind(el, 'mouseenter', function (e) {
                    nav.mouse.add(browser.sanitize(e).currentTarget.id);
                });
                bind(el, 'mouseleave', function (e) {
                    nav.mouse.remove(browser.sanitize(e).currentTarget.id);
                });
            }
        },
        ...
        keyboard: (function () {
            bind(document, 'keydown', function (e) {
                ...
                if (nav.mouse.current() && e.which <= 40 && e.which >= 37) {
                    ...
                    var dir = null;
                    if (e.which === 40 && c['canSlide']('down')) {
                        dir = 'down';
                    } else if (e.which === 39 && c['canSlide']('right')) {
                        dir = 'right';
                    } else if (e.which === 38 && c['canSlide']('up')) {
                        dir = 'up';
                    } else if (e.which === 37 && c['canSlide']('left')) {
                        dir = 'left';
                    }
